...
public class CordovaIdentityPlugin extends CordovaPlugin {
    ...
    public boolean execute(final String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        String tenant = args.getString(0);
        String clientId = args.getString(1);
        String redirectUrl = args.getString(2);
        String policy = args.getString(3);
        String scope = args.length() > 4 ? args.getString(4) : "";
        // UserId is an optional argument
        final String userId = args.length() > 5 ? args.getString(5) : "";

        final IIdentityProvider idp =
                IdentityProviderFactory.Instance.getSignUpIn(tenant,
                        clientId, redirectUrl, policy, scope, this.mGlobalRStrings);
